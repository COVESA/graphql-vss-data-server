...
namespace Vehicle_Cabin_Door_Row1_Right {
    ...
    std::shared_ptr<GraphQLRequestState> state, const IsOpenInputType& input)
{
    ...
    DoorCommand frontRight;
    if (input) {
        frontRight = DoorCommand(Literal::OPEN_DOOR);
    } else {
        frontRight = DoorCommand(Literal::CLOSE_DOOR);
    }
    DoorCommand frontLeft = DoorCommand(Literal::NOTHING);
    DoorCommand rearLeft = DoorCommand(Literal::NOTHING);
...
IMPLEMENTATION_API bool
fromDoorsOpeningStatusToIsOpen(const DoorsStatus& doorsStatus);
